...
let get_handler = (req, res) => {
    ...
    if (param && check) {
        ...
        switch (pathname) {
            ...
            case '/api/aname': functions.getAuditoriumsWithComp1(req, res); break;
            case '/api/wteachers': functions.getPuplitsWithoutTeachers(req, res); break;
            case '/api/fluentAPI': functions.getFluentAPI(req, res); break;
            case '/api/transaction': functions.transaction(req, res); break;
            case '/api/pages': functions.getPages(req, res); break;
            case '/api/pagescount':functions.getCount(req, res); break;
            case '/api/pulpitsWithVladimir': functions.getPulpitsWithVladimir(req, res); break;
            case '/api/auditoriumsSameCount': functions.getAuditoriumsSameCount(req, res); break;
            default: res.writeHead(404); res.end(JSON.stringify({error: "Not found"}));
        }
    }
}
